...
class _LeaveDetailState extends State<LeaveDetail> {
  ...
  void initState() {
    ...
    _leaveService
        .subtitution(globals.getFilterRequest(params: globals.params))
        .then((v) {
    ...
    _leaveService
        .leaveType(globals.getFilterRequest(params: globals.params))
        .then((v) {
      if (v.status == ApiStatus.COMPLETED) {
        if (v.data.data.length > 0) {
          ...
          v.data.data.forEach((i) {
            ...
            if (i.isClosed == false) {
              if (_periodEffective >= _periodStart &&
                  _periodEffective <= _periodFinish) {
    ...
    _formValue?.then((val) async {
      ...
      if (_leaveApplied > 0) {
        AppSnackBar.danger(context,
            'Leave date range is overlapping with the leave that is already availed or applied');
        return;
      }
